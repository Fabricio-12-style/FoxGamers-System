(() => {
  ...
  function badgeModo(modo) {
    const m = {
      ...
      FORZAR_ON:  '<span class="badge" style="background:#d1fae5;color:#065f46;"><i class="fas fa-play-circle mr-1"></i>Forzado on</span>',
      FORZAR_OFF: '<span class="badge" style="background:#fee2e2;color:#9f1239;"><i class="fas fa-stop-circle mr-1"></i>Forzado off</span>',
    };
    return m[modo] || "";
  }
  ...
  function actualizarResumen(datos) {
    ...
    document.getElementById("descuentosActivos").textContent  = datos.filter(d => d.Activo).length;
    document.getElementById("descuentosProximos").textContent = datos.filter(d => calcularEstado(d) === "proximo").length;
  }
  ...
  function aplicarFiltros() {
    ...
    renderizarTabla(listaDescuentosGlobal.filter(d =>
  ...
  window.seleccionarModo = (modo) => {
    const configs = {
      AUTO:       { border:"#00d2ff", bg:"#f0fdff", icon:"#0891b2", infoBg:"#fffbeb", infoBorder:"#fcd34d", infoColor:"#92400e", msg:"El sistema activará y desactivará este descuento automáticamente según las fechas configuradas." },
      FORZAR_ON:  { border:"#10b981", bg:"#f0fdf4", icon:"#10b981", infoBg:"#f0fdf4", infoBorder:"#6ee7b7", infoColor:"#065f46", msg:"El descuento se mantendrá siempre activo sin importar las fechas." },
      FORZAR_OFF: { border:"#ef4444", bg:"#fef2f2", icon:"#ef4444", infoBg:"#fef2f2", infoBorder:"#fca5a5", infoColor:"#9f1239", msg:"El descuento se mantendrá siempre inactivo. El sistema no lo activará aunque esté en fechas." },
    };
  ...
  document.getElementById("dscTipo")?.addEventListener("change", () => {
    ...
    if (label) label.textContent = document.getElementById("dscTipo").value === "PORCENTAJE" ? "%" : "S/";
  });
  ...
  function limpiarFormulario() {
    ...
    const hoy = new Date(), fin = new Date();
  ...
  window.prepararEdicionDsc = async (id) => {
    try {
      const d = await (await fetch(`${API}/descuentos/${id}`)).json();
      ...
      if (d.AplicaA !== "GENERAL") {
        grupo.style.display = "";
        document.getElementById("labelReferencia").textContent = d.AplicaA === "CATEGORIA" ? "Categoría *" : "Producto *";
  ...
  document.getElementById("formDescuento")?.addEventListener("submit", async (e) => {
    ...
    const url    = descuentoEditandoId ? `${API}/descuentos/${descuentoEditandoId}` : `${API}/descuentos`;
    const method = descuentoEditandoId ? "PUT" : "POST";
    try {
      const res  = await fetch(url, { method, headers: { "Content-Type":"application/json" }, body: JSON.stringify(payload) });
      const data = await res.json();
      if (!res.ok || !data.success) return Swal.fire({ icon:"error", title:"Error", text: data.mensaje });
  ...
  window.eliminarDescuento = async (id) => {
    ...
    try {
      const data = await (await fetch(`${API}/descuentos/${id}`, { method:"DELETE" })).json();
